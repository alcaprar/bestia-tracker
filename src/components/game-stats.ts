...
export class GameStats extends LitElement {
  ...
  private createCurrentBalanceChart(): void {
    ...
    const activePlayersWithBalances = this.session!.players
      .filter((p) => p.isActive)
      .map((p) => ({
        name: p.name,
  ...
  private createWinRateChart(): void {
    ...
    const activePlayersStats = this.session!.players
      .filter((p) => p.isActive)
      .map((p) => {
  ...
  private createBestiaCountChart(): void {
    ...
    const activeBestiaCount = this.session!.players
      .filter((p) => p.isActive)
      .map((p) => ({
        name: p.name,
  ...
  render() {
    if (!this.session || this.session.events.length === 0) {
      return html` <div class="empty-state">Nessun evento registrato. Gioca qualche giro per vedere le statistiche.</div> `
    }
        ...
        <div class="stats-grid">
          <div class="chart-card">
            <div class="chart-header">
              ...
              <button class="download-btn" @click=${() => this.downloadChart('balanceProgressionChart', 'evoluzione-bilancio')} title="Scarica grafico">⬇️</button>
            </div>
          ...
          <div class="chart-card">
            <div class="chart-header">
              ...
              <button class="download-btn" @click=${() => this.downloadChart('currentBalanceChart', 'bilancio-attuale')} title="Scarica grafico">⬇️</button>
            </div>
          ...
          <div class="chart-card">
            <div class="chart-header">
              ...
              <button class="download-btn" @click=${() => this.downloadChart('winRateChart', 'vittorie-sconfitte')} title="Scarica grafico">⬇️</button>
            </div>
          ...
          <div class="chart-card">
            <div class="chart-header">
              ...
              <button class="download-btn" @click=${() => this.downloadChart('bestiaCountChart', 'bestia-count')} title="Scarica grafico">⬇️</button>
            </div>
